...
public class PrefferedNeighborsThread implements Runnable {
	...
	/**
	 * @param peerProces
	 *            the peerProces to set
	 */
	public void setPeerProc(PeerProcess peerProces) {
		this.peerProc = peerProces;
	}

	@Override
	public void run() {
		while (!this.getPeerProc().exit) {
			try {
				...
				if (this.getPeerProc().peerInfoVector.size() > 0) {
					...
					if (this.getPeerProc().unchokingIntervalWisePeerDownloadingRate.size() == 0) {
						...
						// as it is a new arraylist, this thread is run for the first time so we do not
						// have previous unchoking interval available thus select any random peers and
						// add them to the preferred neighbors list
						...
						while (indexI < this.getPeerProc().peerInfoVector.size() && this.getPeerProc().NewPrefNeighbors
							...
							Peer p = this.getPeerProc().peerInfoVector.get(ran.nextInt(this.getPeerProc().peerInfoVector.size()));
						...
						while (indexI < CommonPropertiesParser.getNumberOfPreferredNeighbors()) {
							if (this.getPeerProc().unchokingIntervalWisePeerDownloadingRate.isEmpty()) {
								...
								this.getPeerProc().NewPrefNeighbors
										.add(peerProc.unchokingIntervalWisePeerDownloadingRate.poll().getPeer());
							}
							indexI++;
						}
					...
					if (this.getPeerProc().NewPrefNeighbors.size() > 0) {
						...
						Iterator<Peer> iterA = this.getPeerProc().NewPrefNeighbors.iterator();
						while (iterA.hasNext()) {
							Peer p = iterA.next();
						...
						// send choke messages to other who are not present in the new list of preferred neighbors
						...
						String peerIdList = "";
						Iterator<Peer> iterB = this.getPeerProc().PreferedNeighbours.iterator();
						while (iterB.hasNext()) {
							Peer p = iterB.next();
